...
const AddAssignmentModal = (props: any) => {
  ...
  const handleAddAssignment = async () => {
    ...
    try {
        ...
        notify("Thêm bài tập thành công", "success");
        onClose();
        reloadData();
        setIsLoading(false);
    } catch (error) {
        notify("Thêm bài tập thất bại", "error");
        setIsLoading(false);
    }
  }
        ...
        <Modal.Body>
          <div className="flex h-12 items-stretch justify-start gap-4">
            ...
            >
              ...
              <option value={AssignmentType.EXCERCISE}>Bài tập</option>
              <option value={AssignmentType.TEST}>Bài kiểm tra</option>
            </Select>
          </div>
          {/* Scrollable Table */}
